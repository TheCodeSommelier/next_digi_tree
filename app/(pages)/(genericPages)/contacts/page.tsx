...
import EmailForm from "@/app/components/UI/form/EmailForm";
    ...
    <main className="grid grid-cols-1 md:grid-cols-2 items-center justify-between gap-8 md:gap-16 py-16 md:py-26">
      <Suspense>
        <ImageWithOverlay
          ...
          className="h-[500px] md:h-[650px] p-5 md:p-10"
        >
          <div className="flex flex-col justify-between h-full">
            <div className="space-y-3 flex flex-col gap-3 md:gap-6">
              ...
              <p className="max-w-md text-sm md:text-lg text-white">
                Máte další otázky? Neváhejte nás kontaktovat! Přejděte do
                plánování k akcí a získejte přímou diagnostiku vašich procesů.
              </p>
            </div>
            <div className="space-y-6">
              {contactSections.map((section) => (
                <div key={section.id} className="space-y-2">
                  <p className="text-lg md:text-2xl mb-4 font-semibold">
                    {section.label}
                  </p>
